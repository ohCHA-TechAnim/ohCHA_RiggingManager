...
# Description: [v22.01] CORE FINAL.
#              - COMPATIBILITY: Updated to support Refactored SkinningTab (v22.01).
#              - SIGNAL: Explicitly maps signals from split widgets (Hide/Utils/Layer).
...
class RigManagerWindow(OchaBaseWindow):
    def __init__(self):
            ...
            if root:
                ...
                if os.path.exists(settings_path):
                    with open(settings_path, "r", encoding="utf-8") as f:
                        ...
                        rt.print(f"🌍 Initial Language: {initial_lang}")
        except Exception as e:
            rt.print(f"⚠️ Settings Load Error: {e}")
        ...
        self.resize(540, 650) # Slightly taller for split UI
        self._is_sidebar_expanded = True

        # Sync Timer (Max Selection -> UI)
    ...
    def _connect(self):
        ...
        # ========================================================
        # [Tab Connection] Skinning (Refactored)
        # ========================================================
        if "skinning" in self.tabs:
            t = self.tabs["skinning"]
            
            # 1. Hide Manager Signals
            ...
            # 4. Import/Export (Via Aliased Buttons)
            if hasattr(t, "btn_export_skin"):
                t.btn_export_skin.clicked.connect(self._on_export_skin_clicked)
            if hasattr(t, "btn_import_skin"):
                t.btn_import_skin.clicked.connect(self._on_import_skin_clicked)

            # 5. Bone Explorer Signals
            if hasattr(t, "bone_explorer"):
                explorer = t.bone_explorer
                explorer.boneClicked.connect(self._on_skin_select_envelope) # Viewport Sync
                ...
                explorer.removeInfluenceRequested.connect(t._on_remove_bone_influence)

            # 6. Layer Manager Signals
            if hasattr(t, "layer_manager_widget"):
    ...
    def _toggle_side(self):
        ...
        anim_min.setDuration(200); anim_min.setEndValue(target_w); anim_min.setEasingCurve(QEasingCurve.Type.InOutCubic)
        anim_max = QPropertyAnimation(self.sidebar, b"maximumWidth")
        anim_max.setDuration(200); anim_max.setEndValue(target_w); anim_max.setEasingCurve(QEasingCurve.Type.InOutCubic)
        self.anim_group.addAnimation(anim_min); self.anim_group.addAnimation(anim_max); self.anim_group.start()
    ...
    def _update_side_text(self):
        if self._is_sidebar_expanded:
            ...
            self.logo.setText("")
            for b in self.btns.values(): b.setText("")
    ...
    def retranslate(self):
        ...
        for page in self.tabs.values():
            if hasattr(page, 'retranslate_ui'):
                try: page.retranslate_ui()
                except Exception as e: rt.print(f"⚠️ [Retranslate] Error: {e}")
        self._update_side_text()
        QTimer.singleShot(50, lambda: self.adjustSize())
    ...
    def _run_command(self, cmd):
        try:
            if cmd.execute(): rt.print(f"✅ {cmd.name} OK")
            else: rt.print(f"⚠️ {cmd.name} Failed")
        except OchaError as e: QMessageBox.warning(self.window(), translator.get("title_error"), str(e))
        except Exception as e: QMessageBox.critical(self.window(), translator.get("title_error"), str(e)); rt.print(traceback.format_exc())
    ...
    def _on_edit_finalize(self, node, opts):
        ...
        try:
            for c in queue:
                if c.execute(): log.append(f"✅ {c.name}")
                else: ok = False; log.append(f"❌ {c.name}"); break
        except OchaError as e: ok = False; QMessageBox.warning(self.window(), translator.get("title_error"), str(e))
        except Exception as e: ok = False; QMessageBox.critical(self.window(), translator.get("title_error"), str(e))
        rt.print("\n".join(log))
        if ok: QMessageBox.information(self.window(), translator.get("msg_done"), translator.get("msg_finalize_comp") + "\n\n" + "\n".join(log))
        if "edit_mesh" in self.tabs: self.tabs["edit_mesh"]._on_refresh()

    def _get_selected_node(self):
        if rt.selection.count != 1:
            QMessageBox.warning(self.window(), translator.get("title_selection_error"), translator.get("msg_selection_error"))
            return None
        node = rt.selection[0]
    ...
    def _update_skin_layer_ui(self, data: dict):
        ...
        if skin_tab: skin_tab.layer_manager_widget.refresh_ui(data)

    def _on_skin_tab_selected(self, checked=False):
        if skin_controller_instance.is_painting or skin_controller_instance.is_editing_manually: return
        skin_tab = self.tabs.get("skinning")
        if skin_tab:
    ...
    def _on_skin_import_from_selection(self):
        ...
        if QMessageBox.question(self.window(), translator.get("title_import_base"), translator.get("msg_import_base").format(node.name), QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No) == QMessageBox.StandardButton.Yes:
            ...
            if data and data.get('layers'):
                ...
                QMessageBox.information(self.window(), translator.get("title_save_complete"), translator.get("msg_save_complete").format(node.name))
            else:
                skin_tab.layer_manager_widget.set_session_active(False)
    ...
    def _update_bone_explorer(self, show_on_update=False):
        skin_tab = self.tabs.get("skinning")
        if not skin_tab or not skin_controller_instance.node: return
        sort_idx = skin_tab.bone_explorer.sort_combo.currentIndex()
        base_bone_data = skin_controller_instance.get_skin_bone_data_for_ui()
        bone_data = group_controller_instance.get_groups_for_ui(base_bone_data) if sort_idx == 3 else base_bone_data
        skin_tab.bone_explorer.populate_bones(bone_data)
        if show_on_update: skin_tab.bone_explorer.show()

    def _on_bone_explorer_view_changed(self):
        self._update_bone_explorer(show_on_update=True)

    def _on_add_group(self):
        text, ok = QInputDialog.getText(self, translator.get("pop_add_group_title"), translator.get("pop_add_group_label"))
        if ok and text:
            if not group_controller_instance.add_group(text): QMessageBox.warning(self, translator.get("title_error"), translator.get("pop_group_exists").format(text))
            self._update_bone_explorer(True)

    def _on_remove_group(self, group_name):
        if not group_name or group_name == "[Ungrouped]": return
        if QMessageBox.question(self, translator.get("pop_remove_group_title"), translator.get("pop_remove_group_msg").format(group_name)) == QMessageBox.StandardButton.Yes:
    ...
    def _on_rename_group(self, old_name):
        ...
        new_name, ok = QInputDialog.getText(self, translator.get("pop_rename_group_title"), translator.get("pop_rename_group_label"), text=old_name)
        if ok and new_name and new_name != old_name:
            if not group_controller_instance.rename_group(old_name, new_name): QMessageBox.warning(self, translator.get("title_error"), translator.get("pop_rename_fail"))
            self._update_bone_explorer(True)
    ...
    def _on_remove_unused_bones(self):
        if skin_controller_instance.node:
            try:
                if rt.ohCHA_SkinLogic.removeUnusedBones(): self._update_bone_explorer(True)
            except Exception as e: QMessageBox.critical(self, translator.get("title_error"), f"{e}")

    def _on_remove_zero_weights(self):
        if not skin_controller_instance.node: return
        try:
            if rt.ohCHA_SkinLogic.pruneWeights(0.001): rt.print("✅ Zero weights pruned.")
        except Exception as e: rt.print(f"❌ Prune Error: {e}")

    def _on_save_skin(self):
        if not skin_controller_instance.node: return
        if QMessageBox.question(self.window(), translator.get("title_confirm"), translator.get("msg_save_skin_confirm"), QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No) == QMessageBox.StandardButton.Yes:
            try:
                updated_data = skin_controller_instance.collapse_all_layers()
                self._update_skin_layer_ui(updated_data)
                self._on_skin_inject()
                if rt.ohCHA_SkinLogic.openSaveSkinDialog(): rt.print("✅ Save Dialog Opened.")
                else: rt.print("⚠️ Save Dialog Command Failed.")
            except Exception as e: QMessageBox.critical(self.window(), translator.get("title_error"), f"Save Error: {e}")

    def _on_load_skin(self):
        if not skin_controller_instance.node: return
        try:
            if rt.ohCHA_SkinLogic.openLoadSkinDialog(): rt.print("✅ Load Dialog Opened.")
        except Exception as e: rt.print(f"❌ Load Error: {e}")

    def _on_save_bone_list(self):
        if not skin_controller_instance.node: return
        default_name = f"{skin_controller_instance.node.name}_BoneList.json"
        path, _ = QFileDialog.getSaveFileName(self, translator.get("btn_save_bnlist"), default_name, "JSON Files (*.json)")
        if path:
            if skin_controller_instance.save_bone_list_json(path):
                QMessageBox.information(self, translator.get("title_save_complete"), translator.get("msg_bnlist_saved").format(os.path.basename(path)))
    ...
    def _on_load_bone_list(self):
        ...
        if path:
            ...
            if count > 0:
                QMessageBox.information(self, translator.get("title_complete"), translator.get("msg_bnlist_loaded").format(count))
                self._update_bone_explorer(True)
            else: rt.print("⚠️ No bones added.")
    ...
    def _on_find_influencing_bones(self):
        ...
        except NameError: rt.print("❌ Import Error: get_selected_skin_vert_indices missing."); return
        if not vert_indices:
        ...
        try:
            ...
            for v_id in vert_indices:
                ...
                if influencing_ids: all_influences.update(list(influencing_ids))
            ...
            QMessageBox.critical(self, translator.get("title_error"), f"{e}"); traceback.print_exc()
    ...
    def _on_transfer_weights(self):
        ...
        if len(selected_bone_ids) != 1:
            QMessageBox.warning(self, translator.get("title_selection_error"), translator.get("pop_transfer_select_source"))
            return
        source_bone_id = selected_bone_ids[0]
        vert_indices = get_selected_skin_vert_indices(skin_controller_instance.native_skin_mod)
        if not vert_indices:
            QMessageBox.warning(self, translator.get("title_selection_error"), translator.get("pop_transfer_select_verts"))
            return
        ...
        item, ok = QInputDialog.getItem(self, translator.get("pop_transfer_select_target_title"), translator.get("pop_transfer_select_target_label"), items, 0, False)
        if ok and item:
            target_bone = next((b for b in target_candidates if b['name'] == item), None)
            if target_bone:
                try:
                    layer_idx = self._get_active_layer_index()
                    res = skin_controller_instance.transfer_weights_on_layer(source_bone_id, target_bone['id'], layer_idx)
                    if not res: QMessageBox.warning(self, translator.get("title_info"), translator.get("pop_transfer_no_change"))
                except Exception as e: QMessageBox.critical(self, translator.get("title_error"), f"{e}"); traceback.print_exc()
    ...
    def _on_weight_selection(self, action):
        try:
            ...
            if not res: rt.print("⚠️ Selection fail.")
        except Exception as e: rt.print(f"❌ Selection Error: {e}")

    def _on_weight_preset(self, value):
        skin_tab = self.tabs.get("skinning")
        if not skin_tab: return
        selected_bone_ids = skin_tab.bone_explorer.get_selected_bone_ids()
        if len(selected_bone_ids) != 1: rt.print("⚠️ Select exactly ONE bone."); return
        skin_controller_instance.apply_weight_to_active_layer(selected_bone_ids[0], value, "set", ui_layer_index=self._get_active_layer_index())
    ...
    def _on_weight_math(self, operation, value):
        ...
        selected_bone_ids = skin_tab.bone_explorer.get_selected_bone_ids()
        if len(selected_bone_ids) != 1: rt.print("⚠️ Select exactly ONE bone."); return
        skin_controller_instance.apply_weight_to_active_layer(selected_bone_ids[0], value, operation, ui_layer_index=self._get_active_layer_index())

    def _on_weight_clipboard(self, action):
        idx = self._get_active_layer_index()
        if action == "copy": skin_controller_instance.copy_vertex_weights()
        elif action == "paste": skin_controller_instance.paste_vertex_weights(ui_layer_index=idx)

    def _on_weight_smooth(self):
        skin_controller_instance.apply_smooth_to_active_layer(ui_layer_index=self._get_active_layer_index())

    def _on_weight_heal(self):
        skin_controller_instance.apply_smart_heal_to_active_layer(ui_layer_index=self._get_active_layer_index(), tolerance=0.1)
    ...
    def _on_skin_select_envelope(self, bone_id: int):
        if skin_controller_instance.node:
            ...
            except Exception as e: rt.print(f"❌ Select Error: {e}")
    ...
    def _on_skin_paint_commit_toggled(self, ui_index: int):
        ...
        if skin_controller_instance.is_painting:
            ...
            QMessageBox.information(self.window(), translator.get("title_save_complete"), translator.get("msg_paint_saved"))
        else:
            if ui_index < 0: QMessageBox.warning(self.window(), translator.get("title_layer_selection"), translator.get("msg_layer_selection_paint")); return
            selected_bone_ids = skin_tab.bone_explorer.get_selected_bone_ids()
            ...
            if skin_controller_instance.start_painting_session(ui_index, bone_id_to_select):
                QTimer.singleShot(100, lambda: (skin_tab.layer_manager_widget.set_painting_mode(True), self._on_skin_paint_blend_toggled(skin_tab.layer_manager_widget.chk_blend_mode.isChecked())))
            else: QMessageBox.warning(self.window(), translator.get("title_error"), translator.get("msg_error_paint"))
    ...
    def _on_skin_manual_edit_commit_toggled(self, ui_index: int):
        ...
        if skin_controller_instance.is_editing_manually:
            ...
            QMessageBox.information(self.window(), translator.get("title_save_complete"), translator.get("msg_manual_edit_saved"))
        else:
            if ui_index < 0: QMessageBox.warning(self.window(), translator.get("title_layer_selection"), translator.get("msg_layer_selection_manual_edit")); return
            if QMessageBox.question(self.window(), translator.get("title_enter_manual_edit"), translator.get("msg_enter_manual_edit"), QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No) == QMessageBox.StandardButton.Yes:
                if skin_controller_instance.enter_manual_edit_mode(ui_index):
                    skin_tab.layer_manager_widget.set_manual_editing_mode(True)
                    selected_bone_ids = skin_tab.bone_explorer.get_selected_bone_ids()
                    if selected_bone_ids: rt.ohCHA_PaintSession.selectEnvelope(skin_controller_instance.node, selected_bone_ids[0])
                else: QMessageBox.warning(self.window(), translator.get("title_error"), translator.get("msg_error_manual_edit"))
    ...
    def _on_skin_inject(self):
        ...
        if final_weights is None: rt.print("⚠️ [Core] Flattening failed."); return
    ...
    def _on_skin_paint_blend_toggled(self, is_on: bool):
        ...
        try: rt.execute(f"ohCHA_SetPaintBlend {'true' if is_on else 'false'}")
        except Exception as e: rt.print(f"❌ Blend Toggle Error: {e}")

    def _on_skin_paint_options_clicked(self):
        if not skin_controller_instance.is_painting: return
        try: rt.execute("ohCHA_OpenPaintOptions()")
        except Exception as e: rt.print(f"❌ Paint Opt Error: {e}")
    ...
    def _on_skin_collapse_layers(self):
        if QMessageBox.question(self.window(), translator.get("title_collapse_layers"), translator.get("msg_collapse_layers"), QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No) == QMessageBox.StandardButton.Yes:
            updated_data = skin_controller_instance.collapse_all_layers()
            self._update_skin_layer_ui(updated_data)
            self._on_skin_inject()
            QMessageBox.information(self.window(), translator.get("title_complete"), translator.get("msg_collapse_complete"))
    ...
    def _on_skin_update_mask_data(self, ui_index: int, is_remove: bool):
        ...
        if not skin_tab: return
        selected_bone_ids = skin_tab.bone_explorer.get_selected_bone_ids()
        if not selected_bone_ids: QMessageBox.warning(self.window(), translator.get("title_bone_selection"), translator.get("msg_bone_selection_mask")); return
        vert_indices = get_selected_skin_vert_indices(skin_controller_instance.native_skin_mod)
        if not vert_indices and not is_remove: QMessageBox.information(self.window(), translator.get("title_notification"), translator.get("msg_mask_select_success").format(0)); return
        for b_id in selected_bone_ids: skin_controller_instance.update_mask_data(ui_index, b_id, vert_indices, remove=is_remove)
    ...
    def _on_skin_select_mask_verts(self, ui_index: int):
        ...
        if not skin_tab: return
        selected_bone_ids = skin_tab.bone_explorer.get_selected_bone_ids()
        if not selected_bone_ids: QMessageBox.warning(self.window(), translator.get("title_bone_selection"), translator.get("msg_bone_selection_check")); return
        all_mask_verts = set()
        for b_id in selected_bone_ids: all_mask_verts.update(skin_controller_instance.get_mask_verts_for_bone(ui_index, b_id))
        from utils.ohcha_max_utils import select_skin_verts
        if not all_mask_verts:
            QMessageBox.information(self.window(), translator.get("title_notification"), translator.get("msg_no_mask_data"))
    ...
    def _on_export_skin_clicked(self):
        ...
        path, _ = QFileDialog.getSaveFileName(self, translator.get("btn_export_skindata"), default_name, "ohCHA Skin Files (*.ohchaSkin)")
        if path:
            if skin_controller_instance.export_skin_data(path): QMessageBox.information(self, translator.get("title_save_complete"), translator.get("msg_save_complete").format(os.path.basename(path)))

    def _on_import_skin_clicked(self):
        if not skin_controller_instance.node: return
        if QMessageBox.warning(self, translator.get("title_warning"), translator.get("msg_import_warn"), QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No) != QMessageBox.StandardButton.Yes: return
        path, _ = QFileDialog.getOpenFileName(self, translator.get("btn_import_skindata"), "", "ohCHA Skin Files (*.ohchaSkin)")
        if path:
...
if __name__ == "__main__":
    try:
        ...
        rt.print(f"❌ [PySide] Tool Launch Error: {e}")
